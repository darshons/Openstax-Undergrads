    ...
    for i, scene in enumerate(spec.scenes, start=1):
        ...
        try:
            ...
            def write_version(source: str) -> str:
                ...
                path = os.path.join(
                    code_dir, f"scene_{scene.scene_id}_v{scene_version}.py"
                )
            ...
            for round_i in range(MAX_SCENE_REPAIRS):
                ...
                with open(
                    code_path.replace(".py", "_error.log"), "w", encoding="utf-8"
                ) as f:
                    f.write(stderr)
                status.log_event(
                    event="render_failed",
            ...
            for critic_round in range(MAX_CRITIC_ROUNDS):
                ...
                if not ok:
                    ...
                    status.log_event(
                        event="grid_critic_regressed", scene_id=scene.scene_id
                    )
                    break
                video_path = renderer.find_scene_video(media_dir, code_path)
