...
#
# Originally lived in director_experiment.py (a CLI-only script), moved here
# so it's importable as a proper package module from production code
# (solo_clip_pipeline.py / instructor_api.py), not just from sibling scripts
# run directly out of Video_Generation_Pipeline/. director_experiment.py
# re-exports these for backwards compatibility with the existing test
# scripts.
