...
from .logging_utils import OUTPUT_DIR, save_checkpoint_metadata, log_generation
from .prompt_builder import build_veo_prompt, build_clip_prompts
from .veo_api import (
    VEO_MODELS,
    MODEL_KEY,
    RESOLUTION,
...
def run_scene_pipeline(client, scene_id, clip_prompts, reference_images=None, first_clip_seconds=8):
    ...
    try:
        ...
        save_checkpoint_metadata(scene_id, i, checkpoint, e)
...
def run_scenario_pipeline(
    ...
    """
    Iterate over all scenes in the scenario and generate a video for each.
    ...
    models_to_run = list(VEO_MODELS.keys()) if compare_models else [model_key]
