# reference_images.py
#
# Automates the character-portrait + background-reference-image generation
# step that, until now, has been a manual one-off script run by hand before
# any solo-clip generation could start (see the deleted
# scratch_generate_ref_images.py / scratch_generate_bg_image.py from earlier
# in this project). Wraps Image_Generation_Pipeline's generate_characters /
# generate_background with a cache check, so this becomes a real pipeline
# stage - "scenario -> reference images -> rig -> clips -> stitch" - instead
# of a step a human has to remember to run first.
#
...
def _existing_character_image(output_dir: Path, character: dict) -> Path | None:
    ...
    (nurse_maya.png, from Finding the Right Words) and name-based, lowercase
    (maya.png, from the original What Do You Say Next? assets). Recognize
    either as "already generated" so pre-existing hand-set assets are never
    silently regenerated - only genuinely missing images get created, and
    new ones are always written under the character_id convention."""
