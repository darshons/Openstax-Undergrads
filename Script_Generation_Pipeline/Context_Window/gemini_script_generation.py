...
config = types.GenerateContentConfig(
    system_instruction="You are a helpful assistant who summarizes the content of the provided PDF files.",
...
uploaded_pdf = client.files.upload(file=file_path, config=types.UploadFileConfig(display_name=file_name))
...
response = client.models.generate_content(
    ...
    contents=["What is the name of the founder of nursing, and what year did she die in?", uploaded_pdf],
    config=config
)
...
for file in client.files.list():
    ...
    print(f"  File ID/Name: {file.name}") # Will look like 'files/abc123xyz...'
