...
def _validate_inputs(unit_num, unit_name, chapter_num, chapter_name, page_num, page_name):
    """
...
def find_unit_node(toc: dict, unit_num=None, unit_name=None) -> dict:
    ...
    for unit in units:
        hit = (_matches_by_number(unit, unit_num) if unit_num is not None
    ...
    label = f"unit_num={unit_num}" if unit_num is not None else f"unit_name='{unit_name}'"
    raise ValueError(f"Unit not found ({label})\nUnits available:\n" + "\n".join(available))


def get_all_chapters(unit_node: dict) -> list[dict]:
    """Return every chapter node that is a direct child of a unit."""
    return [n for n in (unit_node.get("contents") or []) if n.get("toc_type") == "chapter"]
...
def find_chapter_node(unit_node: dict, chapter_num=None, chapter_name=None) -> dict:
    ...
    for ch in chapters:
        hit = (_matches_by_number(ch, chapter_num) if chapter_num is not None
    ...
    label = (f"chapter_num={chapter_num}" if chapter_num is not None
...
def get_all_pages(chapter_node: dict,
                  unit_title: str = "",
    ...
    def visit(node: dict, path: list[str]):
        ...
        if (node.get("toc_type") == "book-content"
...
def find_page_node(chapter_node: dict, page_num=None, page_name=None,
                   unit_title: str = "", chapter_title: str = "") -> dict:
    """
    ...
    for p in all_pages:
        ...
        hit  = (_matches_page_by_number(node, str(page_num)) if page_num is not None
    ...
    label     = (f"page_num='{page_num}'" if page_num is not None
                 else f"page_name='{page_name}'")
    raise ValueError(
...
def resolve_pages(toc:          dict,
    ...
    """
    _validate_inputs(unit_num, unit_name, chapter_num, chapter_name, page_num, page_name)
    print(f"[4/5] Resolving pages …")
    unit_label    = f"num={unit_num}"    if unit_num    is not None else f"name='{unit_name}'"
    chapter_label = (f"num={chapter_num}" if chapter_num is not None
                     else f"name='{chapter_name}'" if chapter_name is not None
                     else "(all)")
    page_label    = (f"num='{page_num}'"  if page_num   is not None
                     else f"name='{page_name}'"   if page_name   is not None
                     else "(all)")
    ...
    chapters = (
        [find_chapter_node(unit_node, chapter_num=chapter_num, chapter_name=chapter_name)]
        if need_chapter
    ...
    for ch_node in chapters:
        ...
        if need_page:
            result.append(
                find_page_node(ch_node, page_num=page_num, page_name=page_name,
                               unit_title=unit_title, chapter_title=ch_title)
            )
        else:
...
def crawl(book_title:    str,
          unit_num:      int | None = None,
          unit_name:     str | None = None,
          chapter_num:   int | None = None,
          chapter_name:  str | None = None,
          page_num:      str | None = None,
          page_name:     str | None = None) -> dict:
    """
    ...
    toc_pages = resolve_pages(
        toc,
        unit_num=unit_num,       unit_name=unit_name,
        chapter_num=chapter_num, chapter_name=chapter_name,
        page_num=page_num,       page_name=page_name,
    )
...
if __name__ == "__main__":
    ...
    UNIT_NUM     = 1        ; UNIT_NAME     = None
    CHAPTER_NUM  = 1       ; CHAPTER_NAME  = None
    PAGE_NUM     = None   ; PAGE_NAME     = None   # e.g. PAGE_NUM = "1.1"
