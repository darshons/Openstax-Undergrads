...
#internal server issue. Please try again in a few minutes.")
MAX_GENERATION_RETRIES = 4
RETRY_BASE_DELAY_SECONDS = 30  # backoff: 30s, 60s, 120s, 240s
RETRYABLE_ERROR_CODES = {13} # (code 13 INTERNAL, 8 RESOURCE_EXHAUSTED, 14 UNAVAILABLE, or an
...
def _poll_until_done(client, operation):
    ...
    #done=True only means the operation reached a terminal state — NOT that it
    #succeeded. Veo sets error on the operation when generation fails (content
    #policy, transient backend error, etc.) and leaves response=None. So decide
    #success/failure BEFORE announcing the outcome, instead of printing "done!"
    #for a clip that actually failed.
