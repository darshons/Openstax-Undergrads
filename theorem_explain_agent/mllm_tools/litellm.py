...
class LiteLLMWrapper:
    ...
    def __call__(self, messages: List[Dict[str, Any]], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Newer Anthropic models (Fable 5, Opus 4.7/4.8) reject temperature/top_p/top_k
            # with a 400. Omit the sampling param for these; Sonnet 4.6 still accepts it.
            elif re.search(r"(claude-opus-4-[78]|claude-fable-5)", self.model_name):
                response = completion(
                    model=self.model_name,
                    messages=formatted_messages,
                    metadata=metadata,
