...
class CodeGenerator:
    ...
    def _extract_code_with_retries(self, response_text: str, pattern: str, generation_name: str = None, trace_id: str = None, session_id: str = None, max_retries: int = 10) -> str:
        ...
        for attempt in range(max_retries):
            ...
            # Fallback 1: the prompts wrap code in structured tags
            # (<FULL_CORRECTED_CODE>...</FULL_CORRECTED_CODE> for fixes,
            # <CODE>...</CODE> for generation). Pull the code out of those first —
            # this isolates the code region from any <THINKING> prose, then strips
            # an inner ```python fence if present.
            ...
            # Fallback 2: a fenced block with no/other language tag (Claude often
            # uses a bare ``` fence). Prefer the LAST fenced block, since earlier
            # ones may appear inside <THINKING> prose.
            fenced = re.findall(r"```[a-zA-Z]*\s*\n?(.*?)```", response_text, re.DOTALL)
            for block in reversed(fenced):
                if "from manim import" in block or ("class" in block and "Scene" in block):
                    return block
            # Fallback 2: the WHOLE response is raw code (no prose, no fences) —
            # only accept it if it starts at an import/comment and carries no
            # <THINKING>/markdown scaffolding that would break the Python parser.
        ...
        try:
            with open("/tmp/extract_fail_dump.txt", "w") as _f:
