...
class VideoRenderer:
        ...
        while retries < max_retries:
            try:
                ...
                if use_visual_fix_code and visual_self_reflection_func and banned_reasonings:
                    ...
                    # Use a single snapshot image for visual self-reflection.
                    # (The original code referenced self.scene_model here, but
                    # VideoRenderer has no such attribute — so --use_visual_fix_code
                    # crashed with AttributeError for every model. A snapshot works
                    # for all model paths, is enough to catch layout/overlap/z-order
                    # issues, and is faster/cheaper than sending the full video.)
    ...
    def combine_videos(self, topic: str):
        ...
        for scene_num in range(1, scene_count + 1):
            ...
            # Prefer the highest code version, but fall back to whichever version
            # actually produced a rendered mp4. The latest version that compiled is
            # not necessarily the latest version that rendered successfully (e.g. a
            # retry can regenerate code that never renders), so search from newest to
            # oldest for a folder that contains a video.
            folders.sort(key=lambda f: int(f.split("_v")[-1]), reverse=True)

            video_found = False
            subtitles_found = False
            for folder in folders:
                quality_dir = os.path.join(folder, "1080p60")
                ...
                for filename in os.listdir(quality_dir):
                    if filename.endswith('.mp4'):
                        candidate_video = os.path.join(quality_dir, filename)
                    elif filename.endswith('.srt'):
                        candidate_subtitle = os.path.join(quality_dir, filename)
                if candidate_video:
                    scene_videos.append(candidate_video)
                    video_found = True
                    if candidate_subtitle:
                        scene_subtitles.append(candidate_subtitle)
